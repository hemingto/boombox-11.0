...
export async function POST(req: Request) {
  try {
    ...
    const taskIds: TrackingTaskIds = {
      pickup: appointment.onfleetTasks
        .filter(t => t.stepNumber === 1)
        .map(t => t.shortId),
      customer: appointment.onfleetTasks
        .filter(t => t.stepNumber === 2)
        .map(t => t.shortId),
      dropoff: appointment.onfleetTasks
        .filter(t => t.stepNumber === 3)
        .map(t => t.shortId),
      admin: appointment.onfleetTasks
        .filter(t => t.stepNumber === 4)
        .map(t => t.shortId),
    };

    // Format the data for the tracking component using parallel processing
    const deliveryUnits = await Promise.all(
      taskIds.customer.map(async (taskId, index) => {
        // Fetch tasks specific to this delivery unit
        const unitTasks = appointment.onfleetTasks.filter(
          t => t.unitNumber === index + 1
        );
        ...
        const [
          ...
          fetchTaskByShortId(
            unitTasks.find(t => t.stepNumber === 1)?.shortId ?? ''
          ),
          fetchTaskByShortId(
            unitTasks.find(t => t.stepNumber === 2)?.shortId ?? ''
          ),
          fetchTaskByShortId(
            unitTasks.find(t => t.stepNumber === 3)?.shortId ?? ''
          ),
          fetchTaskByShortId(
            unitTasks.find(t => t.stepNumber === 4)?.shortId ?? ''
          ),
        ]);
        ...
        return {
          id: taskId,
          status: (unitDropoffTask?.state === 3
            ? 'complete'
            : unitPickupTask?.state === 2 || unitPickupTask?.state === 3
              ? 'in_transit'
              : 'pending') as 'complete' | 'pending' | 'in_transit',
          unitNumber: index + 1,
          ...
          steps: [
            ...
            },
            {
              ...
              action: {
                label: 'View receipt',
                url: appointment.invoiceUrl || undefined,
              },
              secondaryAction: {
    ...
    const responseValidation =
      TrackingVerifyResponseSchema.safeParse(formattedData);
