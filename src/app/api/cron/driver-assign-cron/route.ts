...
export async function GET(
  req: NextRequest
): Promise<NextResponse<DriverAssignCronResponse>> {
  ...
  try {
    ...
    cutoffTime.setHours(
      cutoffTime.getHours() - DRIVER_NOTIFICATION_TIMEOUT_HOURS
    );

    console.log(
      `📋 Checking for driver assignments older than: ${cutoffTime.toISOString()}`
    );
    ...
    console.log(
      `🔍 Found ${appointmentsToRetry.length} appointments to retry driver assignment`
    );
    ...
    for (const appointment of appointmentsToRetry) {
      try {
        ...
        console.log(
          `✅ Successfully processed appointment ${appointment.id}, retried ${tasksRetried} tasks`
        );
      } catch (error) {
        console.error(
          `❌ Error retrying appointment ${appointment.id}:`,
          error
        );
