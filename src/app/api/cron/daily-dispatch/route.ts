...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!forceDispatch && !dryRun && !isValidDispatchTime()) {
      ...
      console.log(`⏰ Dispatch not allowed before ${PST_DISPATCH_HOUR} PM PST. Next dispatch: ${nextDispatchTime.toISOString()}`);
    ...
    if (dispatchResult.success) {
      ...
      console.log(`📊 Summary: ${dispatchResult.tasksDispatched || 0} tasks dispatched in ${executionTime}ms`);
      
      return NextResponse.json({
        success: true,
        message: dryRun ? 'Daily dispatch simulation completed' : 'Daily dispatch completed successfully',
        data: {
      ...
      if (!dryRun) {
        await sendDispatchFailureNotification(executionDate, errorMessage, executionTime);
      }
    ...
    if (!isDryRun) {
      ...
      await sendDispatchFailureNotification(targetDate, errorMessage, executionTime);
    }
...
async function executeDispatch(options: {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Dispatch API failed: ${response.status} ${response.statusText} - ${JSON.stringify(errorData)}`);
    }
...
async function sendDispatchFailureNotification(
  ...
  try {
    ...
    const templateVariables = {
      ...
      errorMessage,
      impactDescription: 'The automated daily dispatch process failed to complete. This may result in delayed packing supply deliveries as drivers may not be automatically assigned to delivery routes.',
...
5. Monitor subsequent automated dispatch attempts`,
    ...
    for (const email of adminEmails) {
      ...
      if (!result.success) {
        console.error(`Failed to send dispatch failure notification to ${email}:`, result.error);
      }
    }
    
    console.log(`Dispatch failure notification sent to ${adminEmails.length} admin recipients`);
  } catch (error) {
...
async function checkDispatchHealth(): Promise<{
  ...
  try {
    ...
    const nextDispatch = getNextDispatchTime();
    checks.timeConfiguration = nextDispatch instanceof Date && !isNaN(nextDispatch.getTime());
...
export { DailyDispatchCronRequestSchema };