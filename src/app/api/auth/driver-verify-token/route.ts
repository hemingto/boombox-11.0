...
async function decodeToken(token: string): Promise<Record<string, any>> {
  // Try driver_offer first, then driver_reconfirm
  try {
    return await resolveShortToken(token, 'driver_offer');
  } catch {
    return await resolveShortToken(token, 'driver_reconfirm');
  }
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (decodedToken.action === 'reconfirm') {
      ...
      console.log(
        'DEBUG: Reconfirmation token verification for driver',
        decodedToken.driverId,
        'appointment',
        decodedToken.appointmentId
      );
      ...
      if (reconfirmTasks.length === 0) {
        ...
        console.log(
          'DEBUG: Found accepted tasks for this driver:',
        ...
        console.log(
          'DEBUG: No pending or accepted tasks found - returning error'
        );
        return NextResponse.json(
          {
