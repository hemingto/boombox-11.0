...
export async function POST(req: Request) {
  try {
    ...
    const { appointmentId, rating, comment, tipAmount, driverRatings } = validatedData;
    
    console.log('API received parsed data:', { appointmentId, rating, comment, tipAmount, driverRatings });
    ...
    console.log(feedback ? 'Feedback processed successfully' : 'Failed to process feedback');
    ...
    if (tipAmountNumber > 0) {
      try {
        ...
        feedback = await updateFeedbackWithPayment(
          feedback.id,
