...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(
      `[WEBHOOK] Raw body preview: ${text.substring(0, 500)}${text.length > 500 ? '...' : ''}`
    );
    ...
    console.log(
      `[WEBHOOK] Parsed data keys: ${Object.keys(webhookData).join(', ')}`
    );
    ...
    const isHaulJobTask = jobType === 'haul_job';

    console.log(
      `[WEBHOOK] Job type: ${jobType || 'not specified (storage unit)'}`
    );
    console.log(`[WEBHOOK] Is packing supply task: ${isPackingSupplyTask}`);
    console.log(`[WEBHOOK] Is haul job task: ${isHaulJobTask}`);

    // Check for duplicate webhook (idempotency) for storage unit and haul job tasks
    ...
    if (!isPackingSupplyTask && taskDetails?.shortId) {
      ...
      if (onfleetTask?.lastProcessedWebhookTime === BigInt(time)) {
        console.log(
          `[WEBHOOK] Skipping duplicate webhook: ${triggerName} at ${time} for task ${taskDetails.shortId}`
        );
        return NextResponse.json({
          success: true,
    ...
    if (isHaulJobTask) {
      console.log(`[WEBHOOK] >>> Processing haul job webhook`);

      if (onfleetTask?.haulJobId) {
        const { haulJobService } = await import(
          '@/lib/services/HaulJobService'
        );
        const { HaulJobStatus } = await import('@prisma/client');

        if (triggerName === 'taskCompleted' || triggerName === 'taskStarted') {
          const stepNumber = onfleetTask.stepNumber;

          if (stepNumber === 1) {
            await haulJobService.updateHaulJobStatus(
              onfleetTask.haulJobId,
              HaulJobStatus.IN_TRANSIT
            );
            console.log(
              `[WEBHOOK] Haul job ${onfleetTask.haulJobId} → IN_TRANSIT (loading complete)`
            );
            ...
            console.log(
              `[WEBHOOK] Haul job ${onfleetTask.haulJobId} → UNLOADING (arrived)`
            );
          } else if (stepNumber === 3) {
            await haulJobService.updateHaulJobStatus(
              onfleetTask.haulJobId,
              HaulJobStatus.COMPLETED
            );
            console.log(
              `[WEBHOOK] Haul job ${onfleetTask.haulJobId} → COMPLETED (unloaded)`
            );
          }
        }
        ...
        return NextResponse.json({
          success: true,
          message: 'Haul job webhook processed',
        });
      } else {
        console.log(`[WEBHOOK] Haul job task found but no haulJobId linked`);
        return NextResponse.json({
          success: true,
          message: 'No haul job linked to task',
        });
      }
    }
    ...
    if (isPackingSupplyTask) {
      ...
      const result = await PackingSupplyWebhookService.handle(
        validationResult.data
      );
      console.log(
        `[WEBHOOK] PackingSupply result:`,
        JSON.stringify(result, null, 2)
      );

      if (result.status && !result.success) {
        console.log(
          `[WEBHOOK] PackingSupply returned error status: ${result.status}`
        );
        return NextResponse.json(
          { error: result.error },
      ...
      const result = await StorageUnitWebhookService.handle(
        validationResult.data
      );
      console.log(
        `[WEBHOOK] StorageUnit result:`,
        JSON.stringify(result, null, 2)
      );

      if (!result.success) {
        console.log(
          `[WEBHOOK] StorageUnit processing failed: ${result.message}`
        );
      ...
      if (onfleetTask) {
        ...
        console.log(
          `[WEBHOOK] Updated lastProcessedWebhookTime to ${time} for task ${onfleetTask.shortId}`
        );
      }
