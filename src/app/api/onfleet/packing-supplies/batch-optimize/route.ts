...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `Starting batch optimization for date: ${optimizationDate.toDateString()}`
    );
    ...
    if (ordersNeedingTasks.length > 0) {
      console.log(
        `Creating Onfleet tasks for ${ordersNeedingTasks.length} orders without tasks`
      );
      ...
      for (const order of ordersNeedingTasks) {
        const taskData = {
          ...
          completeBefore: order.deliveryWindowEnd
            ? order.deliveryWindowEnd.getTime()
            : endOfDay.getTime(),
          completeAfter: order.deliveryWindowStart
            ? order.deliveryWindowStart.getTime()
            : startOfDay.getTime(),
          ...
          metadata: [
            ...
            { name: 'batch_optimization', type: 'boolean', value: true },
            {
              name: 'batch_date',
              type: 'string',
              value: optimizationDate.toISOString().split('T')[0],
            },
            {
              name: 'optimization_timestamp',
              type: 'number',
              value: Date.now(),
            },
          ],
        ...
        try {
          ...
          console.log(
            `Created Onfleet task ${task.shortId} for order ${order.id}`
          );
        } catch (error) {
          console.error(
            `Failed to create Onfleet task for order ${order.id}:`,
            error
          );
    ...
    const optimizedRoutes = await routeOptimizer.optimizeOrders(
      deliveryOrders,
      optimizationDate
    );
    console.log(
      `Simple route optimization created ${optimizedRoutes.length} routes`
    );
    console.log(
      'Delivery orders passed to optimizer:',
      JSON.stringify(deliveryOrders, null, 2)
    );
    console.log(
      'Optimized routes returned:',
      JSON.stringify(optimizedRoutes, null, 2)
    );
      ...
      return NextResponse.json<ApiResponse<null>>(
        {
          ...
          error: {
            ...
            message:
              'BOOMBOX_PACKING_SUPPLY_DELIVERY_DRIVERS team ID is not configured',
            details: {
              targetDate: optimizationDate.toISOString().split('T')[0],
    ...
    for (const route of optimizedRoutes) {
      try {
        ...
        console.log(
          `Created Route Plan ${routePlan.shortId} with ${route.orders.length} stops`
        );
      } catch (error) {
        console.error(
          `Failed to create route plan for ${route.routeId}:`,
          error
        );
        console.error(
          'Full error details:',
    ...
    const successfulRoutes = createdRoutes.filter(
      (route: any) => !route.status || route.status !== 'failed'
    );
    ...
    for (const route of successfulRoutes) {
      try {
        ...
        if (driverOfferResponse.ok) {
          ...
          console.error(
            `Failed to send driver offer for route ${route.routeId}:`,
            errorData.message
          );
        }
      } catch (error: any) {
        ...
        console.error(
          `Error sending driver offer for route ${route.routeId}:`,
          error
        );
      }
    }

    const failedRoutes = createdRoutes.filter(
      (route: any) => route.status === 'failed'
    );
    ...
    >({
      ...
      data: {
        ...
        failedRoutes: failedRoutes.map((r: any) => ({
          routeId: r.routeId,
          error: r.error,
        })),
        driverOffers: driverOfferResults,
        offersSuccessful: driverOfferResults.filter(
          r => r.status === 'offer_sent'
        ).length,
        offersFailed: driverOfferResults.filter(r => r.status !== 'offer_sent')
          .length,
    ...
    return NextResponse.json<ApiResponse<null>>(
      {
        ...
        error: {
          code: 'INTERNAL_ERROR',
          message:
            error.message || 'Unknown error occurred during optimization',
        },
      },
      { status: 500 }
    );
...
async function createRoutesFromOptimization(
  ...
  for (let i = 0; i < optimizedRoutes.length; i++) {
    ...
    if (optimizedRoute.tasks) {
      for (
        let taskIndex = 0;
        taskIndex < optimizedRoute.tasks.length;
        taskIndex++
      ) {
    ...
    routes.push(route);
    console.log(
      `Created route ${routeId} with ${taskCount} stops, ${routeDistance}mi, ${Math.round(routeTime / 60)}min`
    );
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = {
      ...
      optimized: orders.filter(
        o => o.status === 'Optimized' || o.status === 'Ready for Assignment'
      ).length,
