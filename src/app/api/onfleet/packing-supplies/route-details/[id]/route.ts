  ...
  try {
    ...
    if (!paramsValidation.success) {
      return NextResponse.json(
        {
          error: 'Invalid route ID parameter',
    ...
    if (
      route.driverOfferStatus === 'accepted' &&
      route.driverId !== tokenData.driverId
    ) {
      return NextResponse.json(
    ...
    if (
      ...
      now > route.driverOfferExpiresAt.getTime()
    ) {
      return NextResponse.json(
