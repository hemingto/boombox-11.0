...
export async function GET(
  request: NextRequest
): Promise<NextResponse<SearchMovingPartnersResponse | { error: string }>> {
  try {
    ...
    console.log('[API/moving-partners/search] Received params:', {
      dateString,
      timeString,
      excludeAppointmentId: excludeAppointmentIdStr,
    });
    ...
    const validationResult =
      SearchMovingPartnersRequestSchema.safeParse(queryData);
    ...
    // Map response to handle nullable fields and ensure proper types
    const mappedResponse = availablePartnersWithCapacity.map(partner => ({
      ...partner,
      email: partner.email || '', // Convert null to empty string
      phoneNumber: partner.phoneNumber || '', // Convert null to empty string
      availability: partner.availability.map(avail => ({
        id: avail.id,
        ...
        // Remove movingPartnerId, createdAt, updatedAt from availability
      })),
    }));
    ...
    console.error(
      '[API/moving-partners/search] Error fetching moving partners:',
      error
    );
