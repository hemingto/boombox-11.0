 ...
 * - Works alongside useSessionMonitor hook for auto-redirect on session expiry
 *
 * ROUTE PROTECTION:
...
export async function middleware(req: NextRequest) {
  ...
  console.log(
    ...
    'Token:',
    token ? `${token.accountType} (${token.id})` : 'null'
  );
  ...
  if (token && req.nextUrl.pathname === '/login') {
    ...
    console.log(
      '[middleware] Logged-in user accessing /login, redirecting based on accountType:',
      accountType
    );
    ...
    if (accountType === 'customer' || accountType === 'user') {
      ...
      return NextResponse.redirect(
        new URL(`/service-provider/driver/${userId}`, req.url)
      );
  ...
  if (
    req.nextUrl.pathname.startsWith('/service-provider/driver') &&
    ...
    try {
      ...
      if (requestedDriverId && token.id !== requestedDriverId) {
        ...
        return NextResponse.redirect(
          new URL(`/service-provider/driver/${token.id}`, req.url)
        );
      }
    } catch (error) {
      console.error('Error parsing token:', error);
  ...
  if (req.nextUrl.pathname.startsWith('/service-provider/mover')) {
    ...
    try {
      ...
      if (requestedMoverId && token.id !== requestedMoverId) {
        ...
        return NextResponse.redirect(
          new URL(`/service-provider/mover/${token.id}`, req.url)
        );
      }
    } catch (error) {
      console.error('Error parsing token:', error);
  ...
  if (req.nextUrl.pathname.startsWith('/admin')) {
    ...
    if (
      ...
      const accountType =
        typeof token?.accountType === 'string'
          ? token.accountType.toUpperCase()
          : '';
      if (accountType === 'ADMIN') {
        // Redirect to admin dashboard if already logged in as admin
        console.log(
          '[middleware] Already logged in as admin, redirecting to /admin'
        );
    ...
    if (accountType !== 'ADMIN') {
      console.log(
        '[middleware] User is not admin (accountType:',
        token.accountType,
        '), redirecting to /'
      );
    ...
    if (
      ...
      console.log(
        ...
        token.role,
        'not SUPERADMIN, redirecting to /admin'
      );
      return NextResponse.redirect(new URL('/admin', req.url));
    }
  }
