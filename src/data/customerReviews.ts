 ...
 * Supports both hardcoded fallback reviews and live Google Places API reviews
 */
export interface CustomerReview {
  ...
  rating?: number;
  verified?: boolean;
  /** Reviewer's profile photo URL (from Google Places API) */
  photoUrl?: string;
...
export const CUSTOMER_REVIEWS: CustomerReview[] = [
  {
    ...
    date: 'August 2020',
    description:
      "I've used Boombox pickup & dropoff multiple times since the pandemic started. The Boombox crew has been wonderful every time and they. I've used Boombox pickup & dropoff multiple times been wonderful every time and they.",
  },
  {
 ...
 */
export function getReviewByCustomer(
  customerName: string
...
export function getReviewsByYear(year: number): CustomerReview[] {
  return CUSTOMER_REVIEWS.filter(review =>
    review.date.includes(year.toString())
  );
}
