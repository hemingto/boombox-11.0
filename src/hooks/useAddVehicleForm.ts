...
export const HAULER_VEHICLE_MAKES = [
  'Freightliner',
  'Kenworth',
  'Peterbilt',
  'International',
...
export function useAddVehicleForm({
  ...
  const updatePhoto = useCallback(
    (field: keyof VehiclePhotos, file: File | null) => {
  ...
  const validateForm = useCallback((): {
    ...
    // Validate trailer hitch (drivers/movers) or boombox capacity (haulers)
    if (userType === 'hauler') {
      if (formData.boomboxCapacity === null || formData.boomboxCapacity < 3) {
        newErrors.boomboxCapacity =
          'Please enter how many Boomboxes you can haul';
        isValid = false;
        if (!firstErrorField) firstErrorField = 'boomboxCapacity';
      }
    } else {
      if (!formData.hasTrailerHitch) {
        newErrors.trailerHitch =
          'Please select whether your vehicle has a trailer hitch';
        isValid = false;
    ...
    if (!photos.autoInsurancePhoto) {
      newErrors.autoInsurancePhoto =
        'Please upload your auto insurance document';
      isValid = false;
  ...
  const handleSubmit = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      if (photoEntries.length > 0) {
        ...
        for (const [fieldName, file] of photoEntries) {
          try {
            ...
            const url = await VehicleService.uploadVehiclePhoto(
              userId,
              userType,
              file,
              fieldName
            ...
            if (fieldName === 'autoInsurancePhoto') {
              errorMessage =
                'Failed to upload your insurance document. Please try again or email us at help@boomboxstorage.com';
            } else if (fieldName === 'frontVehiclePhoto') {
              errorMessage =
                'Failed to upload your front vehicle photo. Please try again or email us at help@boomboxstorage.com';
            } else if (fieldName === 'backVehiclePhoto') {
              errorMessage =
                'Failed to upload your back vehicle photo. Please try again or email us at help@boomboxstorage.com';
            } else {
      ...
      const vehicleData = {
        ...
        hasTrailerHitch:
          userType !== 'hauler'
            ? formData.hasTrailerHitch === 'Yes'
            : undefined,
        boomboxCapacity:
          userType === 'hauler' ? formData.boomboxCapacity : undefined,
      ...
      if (onSuccess) {
        ...
        const redirectMap: Record<UserType, string> = {
          driver: `/service-provider/driver/${userId}/vehicle`,
          mover: `/service-provider/mover/${userId}/vehicle`,
          hauler: `/service-provider/hauler/${userId}/vehicle`,
        };
