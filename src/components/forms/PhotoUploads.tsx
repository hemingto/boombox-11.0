...
export interface PhotoUploadsProps extends UsePhotoUploadOptions {
  ...
  /**
   * When true, always show the upload button instead of large file previews after upload.
   * Useful when previews are rendered separately (e.g. a compact document list).
   */
  hidePreview?: boolean;
}
...
const PhotoUploads: React.FC<PhotoUploadsProps> = ({
  ...
  const renderIcon = () => {
    ...
    if (error) {
      ...
      const errorClassName = existingClassName.replace(
        /text-\S+/,
        'text-red-100'
      );
  ...
  const renderModalContent = () => {
    if (uploadState === 'selecting') {
      return (
        <div className="fixed inset-0 bg-primary bg-opacity-50 flex items-center justify-center z-50">
          <div
            className="bg-surface-primary rounded-lg shadow-xl w-full max-w-xl"
            role="dialog"
            aria-modal="true"
            aria-labelledby="modal-title-selecting"
          >
            <div className="flex justify-between items-center p-4 border-b">
              <h3 id="modal-title-selecting" className="text-lg font-medium">
                {photoUploadTitle}
              </h3>
              <button
    ...
    if (
      ...
      return (
        <div className="fixed inset-0 bg-primary bg-opacity-50 flex items-center justify-center z-50">
          <div
            className="bg-surface-primary rounded-lg shadow-custom-shadow w-full max-w-xl"
            role="dialog"
            aria-modal="true"
            aria-labelledby="modal-title-preview"
          >
            <div className="flex justify-between items-center p-4 border-b">
              <h3 id="modal-title-preview" className="text-lg font-medium">
                {photoUploadTitle}
              </h3>
              <button
                onClick={handleCloseModal}
  ...
  return (
    <div className={cn('w-full', className)}>
      ...
      >
        {uploadState === 'done' && selectedFiles.length > 0 && !hidePreview ? (
          <div className="w-full space-y-3">
            {selectedFiles.map((file, index) => (
              <div key={index} className="relative w-full">
                {file.type.includes('pdf') ||
                file.name.toLowerCase().endsWith('.pdf') ? (
                  <div className="w-full flex items-center justify-center bg-surface-tertiary p-10 rounded-md">
                    <div className="flex flex-col items-center justify-center p-4">
                  ...
                  <Image
                    src={
                      index === 0
                        ? displayedImageUrl || URL.createObjectURL(file)
                ...
                <button
                  ...
                  disabled={disabled}
                  aria-label={`Delete uploaded file ${index + 1}`}
                >
                  <TrashIcon className="w-5 h-5" />
                </button>
              </div>
