...
interface RecentBlogPostsProps {
  ...
  limit?: number;
  /** Slug of the current post to exclude from the list */
  excludeSlug?: string;
}
    ...
    async function fetchRecentPosts() {
      try {
        ...
        const response = await fetch(
          `/api/blog/posts?limit=${fetchLimit}&page=1`
        );
        ...
        const allPosts: LegacyBlogPost[] = data.posts || [];
        const filtered = excludeSlug
          ? allPosts
              .filter(p => p.link !== `/blog/${excludeSlug}`)
              .slice(0, limit)
          : allPosts;
        setPosts(filtered);
      } catch (err) {
