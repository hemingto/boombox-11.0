...
export const PaymentInvoices: React.FC<PaymentInvoicesProps> = ({ userId }) => {
    ...
    const fetchPaymentHistory = async () => {
      try {
        ...
        const response = await fetch(
          `/api/payments/payment-history?userId=${userId}`
        );
  ...
  const handleViewInvoice = async (invoice: Invoice) => {
    try {
      ...
      const response = await fetch(
        `/api/payments/invoice-pdf?paymentIntentId=${invoice.id}&type=${type}`
      );

      if (!response.ok) {
        throw new Error(
          `Failed to fetch ${type === 'invoice' ? 'invoice PDF' : 'payment receipt'}`
        );
      }
    ...
    <div className="max-w-5xl lg:px-16 px-6 mx-auto">
      ...
      <div className="bg-white rounded-md shadow-custom-shadow">
        {paginatedInvoices.map(invoice => (
          <div
            key={invoice.id}
            className="flex items-center justify-between py-4 border-b border-border last:border-none px-4"
          >
            <div className="flex items-center space-x-4">
              <div className="flex items-center justify-center w-14 h-14 rounded-md bg-surface-tertiary">
                <BanknotesIcon className="w-7 h-7 text-text-secondary" />
              </div>
              <div>
                ...
                <p className="text-xs text-text-secondary">
                  {formatCardBrand(invoice.cardBrand)} **** {invoice.last4}
                </p>
              </div>
            </div>
            <div className="flex items-center text-right gap-2">
              ...
              <p className="text-sm font-semibold w-16 text-center text-text-primary">
                {invoice.amount}
              </p>
              <button
