  ...
  <Modal open={isOpen} onClose={onClose} title={title} size="full">
   <div className="flex flex-col h-full">
    ...
    <div className="mb-3 flex-shrink-0">
     ...
     {lastAccessedDate && (
      <p className="text-sm text-text-primary">Last Accessed: {lastAccessedDate}</p>
     )}
    </div>

    {/* Image and Description Section */}
    <div className="flex-1 flex flex-col min-h-0">
     <div className="relative w-full flex-1 min-h-0 bg-surface-tertiary rounded-md overflow-hidden">
      {/* Image */}
      {!isImageBroken ? (
       <Image
        src={currentImage}
       ...
       <div className="flex items-center justify-center w-full h-full text-sm text-text-secondary text-center">
        Image not available
       </div>
      )}
      ...
      >
       ...
       <textarea
        ...
        className="w-full h-full resize-none bg-transparent focus:outline-none placeholder:text-text-inverse text-text-inverse pb-6"
        maxLength={1000}
       ...
       <span
        className={`absolute bottom-2 right-3 text-xs ${
         localDescription.length === 1000 ? 'text-status-error' : 'text-text-inverse'
        }`}
       >
      ...
      <button
       ...
       className="absolute top-3 right-3 flex items-center text-xs rounded-full p-2.5 bg-surface-tertiary sm:hover:bg-surface-disabled active:bg-surface-disabled"
     ...
     {/* Pagination Controls */}
     {totalPages > 1 && (
      <div className="relative flex justify-center items-center mt-4 flex-shrink-0">
       {/* Previous Button */}
       <button
       ...
       {/* Page Counter */}
       <div className="text-sm text-text-tertiary" aria-live="polite" aria-atomic="true">
