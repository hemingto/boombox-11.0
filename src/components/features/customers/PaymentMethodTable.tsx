...
useEffect(() => {
 ...
 const handleRemove = async (id: string) => {
  ...
  const removedMethod = paymentMethods.find(m => m.id === id);
  const updatedMethods = paymentMethods.filter(method => method.id !== id);
  
  // If removing default card and there are other cards, set the first one as default
  ...
  try {
   ...
   // If API returned a new default, update accordingly
 ...
 const handleSetAsDefault = async (id: string) => {
  ...
  // Set loading indicator for this specific card
  setLoadingCardId(id);
  setShowOptions(null);
  ...
  <div className="max-w-5xl lg:px-16 px-6 mx-auto mb-12">
     ...
     {paymentMethods.map((method) => (
      ...
      >
       ...
       <div className="relative flex items-center space-x-2">
        ...
        <button 
         onClick={() => setShowOptions(showOptions === method.id ? null : method.id)}
         disabled={loadingCardId === method.id}
        >
    ...
    {showDeleteConfirmation && (
     ...
     >
      {paymentMethods.length === 1 ? (
       // Show error message when only one card exists
       <>
        <div className="bg-status-bg-error p-4 border border-border-error rounded-md">
         <h4 className="text-base text-status-error font-semibold mb-2">
          You only have one card on file
         </h4>
         <p className="text-sm text-status-error">
          You cannot remove this card since you must keep at least one card on file.
         </p>
        </div>
       ...
       // Show normal delete confirmation when multiple cards exist
       <>
     ...
     {/* Add Card Button */}
     <button
      className="flex items-center justify-between w-full py-4 sm:hover:bg-surface-tertiary active:bg-surface-disabled px-6"
   ...
   >
    ...
    <div className="flex justify-end">
     ...
     >
      {isAddingCard ? 'Adding...' : 'Add Payment Details'}
     </button>
    </div>
   </Modal>
  </div>
