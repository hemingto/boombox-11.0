...
export function AppointmentCard({
  ...
  const handleEditClick = () => {
    ...
    router.push(`/user-page/${userId}/edit-appointment?${params.toString()}`);
  };
  ...
  const calculateTotal = () => {
    ...
    return (
      (monthlyStorageRate || 0) * (numberOfUnits || 1) +
      (monthlyInsuranceRate || 0) * (numberOfUnits || 1) +
      (loadingHelpPrice || 0)
    );
  };
    ...
    <div className="bg-surface-primary mb-4 rounded-md shadow-custom-shadow">
        ...
        {status !== 'Complete' && trackingUrl && (
          <>
            ...
            <a
              ...
              className="absolute top-4 right-4 z-10 bg-surface-primary rounded-md px-3 py-2 flex items-center gap-2 shadow-md hover:bg-surface-secondary transition-colors"
            >
      ...
      <div className="border-t border-border">
        ...
        <div className="border-b border-border">
          <button
            ...
            className="px-4 py-4 flex items-center justify-between w-full text-text-primary sm:hover:bg-surface-secondary active:bg-surface-tertiary transition"
          >
                    ...
                    <div className="space-y-4">
                      {appointmentType === 'Storage Unit Access' || appointmentType === 'End Storage Term' ? (
                          ...
                          {requestedStorageUnits.length > 0 ? (
                            requestedStorageUnits.map((unit) => (
                              <div key={unit.id} className="flex justify-between items-center">
                                ...
                                <span className="text-sm font-medium">{formatCurrency(accessStorageUnitPricing)}</span>
                              </div>
                            ))
                          ) : (
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-sm font-medium">
                                {formatCurrency(accessStorageUnitPricing * numberOfUnits)}
                              </span>
                            </div>
                          )}
                        </>
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-sm font-medium">
                            {formatCurrency((monthlyStorageRate || 0) * (numberOfUnits || 1))}/mo
                          </span>
                        </div>
                      )}
                      <div className="flex justify-between items-center">
                        ...
                        <div>
                          ...
                          <span className="text-sm font-medium">{formatCurrency(loadingHelpPrice || 0)}/hr</span>
                        </div>
                      </div>
                      {appointmentType !== 'Storage Unit Access' && appointmentType !== 'End Storage Term' && (
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-sm font-medium">
                            {formatCurrency((monthlyInsuranceRate || 0) * (numberOfUnits || 1))}/mo
                          </span>
                        </div>
                      )}
        ...
        {!trackingUrl && (
          <>
            ...
            <button
              ...
              className="px-4 py-4 flex items-center justify-between w-full text-text-primary rounded-b-md sm:hover:bg-surface-secondary active:bg-surface-tertiary transition border-t border-border"
            >
              <div className="flex items-center">
                <PencilSquareIcon className="w-5 h-5 mr-2 text-text-primary" />
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <p className="mt-2 mb-4 text-lg text-text-primary">Tell us why you need to cancel</p>
            <div className="space-y-2">
              {CANCELLATION_OPTIONS.map((option) => (
                <label
                  key={option}
                  className="flex items-center p-3 border border-border rounded-md hover:bg-surface-secondary cursor-pointer transition"
                >
                  <input
                    type="radio"
          ...
          <div className="flex justify-end gap-3 pt-4">
            <button
              ...
              className="px-5 py-2.5 text-sm font-semibold text-text-primary border border-border rounded-md hover:bg-surface-secondary transition"
