    ...
    <div className="relative rounded-2xl shadow-sm max-w-3xl mx-auto">
     ...
     <div className="absolute inset-y-0 right-0 flex items-center pr-3">
      <button
       ...
       className={`p-2 rounded-full text-text-inverse bg-primary hover:bg-primary-hover disabled:opacity-50 disabled:cursor-not-allowed ${loading ? 'animate-pulse' : ''}`}
      >
       {loading ? (
   ...
   <div className="flex flex-wrap gap-3 justify-center max-w-3xl mx-auto">
    {exampleQueries.map((eq, index) => (
     <button
      ...
      className="flex items-center gap-2 px-4 py-2 rounded-full bg-surface-tertiary hover:bg-surface-secondary text-sm text-text-primary"
     >
       ...
       <table className="min-w-full divide-y divide-border">
        ...
        <tbody className="divide-y divide-border bg-surface-primary">
         {result.results.map((row, index) => (
          <tr key={index} className="hover:bg-surface-tertiary">
