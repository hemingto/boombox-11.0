 ...
 * 
 * API ROUTES USED:
 * - PATCH /api/admin/tasks/update-location/[usageId] - Update warehouse location
 * 
...
export function UpdateLocationPage({ taskId }: UpdateLocationPageProps) {
 ...
 // Extract usage ID from task ID (format: update-location-{usageId})
 const usageId = taskId.replace('update-location-', '');
 ...
 const handleSubmit = async () => {
  ...
  try {
   const response = await fetch(`/api/admin/tasks/update-location/${usageId}`, {
    method: 'PATCH',
    headers: {
      ...
      <div className="mt-8">
       ...
       <div className="flex justify-end mt-20">
        <Button
         ...
         disabled={!warehouseLocation.trim()}
