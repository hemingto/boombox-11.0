 ...
 * - GET /api/appointments/[id]/getDriverByUnit - Get driver for specific unit
 * - POST /api/upload/unit-pickup-photos - Upload trailer photos to Cloudinary
 * - POST /api/admin/appointments/[id]/assign-storage-units - Assign unit to appointment
 * 
...
export function AssignStorageUnitPage({ taskId }: AssignStorageUnitPageProps) {
    ...
    const fetchDriverForUnit = async () => {
      ...
      try {
        const response = await fetch(
          `/api/appointments/${appointmentId}/getDriverByUnit?unitNumber=${unitIndex}`
        );
        if (!response.ok) {
    ...
    try {
      ...
      for (const photo of trailerPhotos) {
        ...
        const uploadResponse = await fetch(
          `/api/upload/unit-pickup-photos?entityType=trailer&appointmentId=${appointment?.id}`,
          {
            method: 'POST',
            body: formData,
          }
  ...
  const handleAssignUnit = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/appointments/${appointment.id}/assign-storage-units`, {
        method: 'POST',
        headers: {
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="bg-status-bg-warning rounded-lg p-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="relative bg-status-warning rounded-full h-12 w-12 overflow-hidden flex items-center justify-center">
                ...
                <div className="flex items-center">
                  <div className="relative bg-status-warning rounded-full h-12 w-12 overflow-hidden mr-3 flex items-center justify-center">
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-end pt-4">
                ...
                >
                  {isSubmitting ? 'Assigning...' : 'Assign Storage Unit'}
                </button>
              </div>
            </div>
          </div>
