 ...
 * - PATCH /api/admin/appointments/[id]/storage-unit-return - Update appointment
 * 
...
export function StorageUnitReturnPage({ taskId }: StorageUnitReturnPageProps) {
  ...
  const [isUnitEmpty, setIsUnitEmpty] = useState<string | null>(null);
  ...
  const [showUnitEmptyError, setShowUnitEmptyError] = useState(false);
    ...
    if (task?.appointmentId) {
      ...
      try {
        ...
        if (appointmentType === 'Initial Pickup' || appointmentType === 'Additional Storage') {
          requestBody.isUnitEmpty = isUnitEmpty === 'Yes';
        }

        // Update appointment and create damage report if needed
        const response = await fetch(
          `/api/admin/appointments/${task.appointmentId}/storage-unit-return`,
          {
            method: 'PATCH',
            headers: {
      ...
      <div className="w-full sm:max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-surface-primary rounded-lg shadow-sm">
          <div className="p-6 space-y-6">
            ...
            <div className="bg-status-bg-pending rounded-lg p-6">
              <div className="flex justify-between">
                <div className="flex items-center gap-3">
                  <div className="bg-status-pending rounded-full p-3">
            ...
            {/* Initial Pickup and Additional Storage - Is unit empty question */}
            {(appointmentType === 'Initial Pickup' || appointmentType === 'Additional Storage') && (
              <div>
                <p className="text-text-primary mb-4">Did {task.storageUnitNumber} return empty?</p>
                <YesOrNoRadio
                  value={isUnitEmpty}
                  onChange={(value) => {
                    setIsUnitEmpty(value);
                    setShowUnitEmptyError(false);
                  }}
                  hasError={showUnitEmptyError}
                />
                {showUnitEmptyError && <p className="form-error">Please indicate if the unit is empty</p>}
              </div>
            )}
