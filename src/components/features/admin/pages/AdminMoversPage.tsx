...
export function AdminMoversPage() {
  ...
  // Modal states
  const [selectedMover, setSelectedMover] = useState<MovingPartner | null>(
    null
  );
  ...
  const actionFilterItems = [
    {
      id: 'approve_movers',
      label: 'Approve Moving Partners',
      checked: actionFilters.approve_movers,
    },
  ];
  ...
  const handleApproveConfirm = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/moving-partners/${selectedMover.id}/approve`,
        {
          method: 'POST',
        }
      );
  ...
  const renderCellContent = (
    ...
    switch (column.id) {
      ...
      case 'phoneNumber':
        return mover.phoneNumber
          ? formatPhoneNumberForDisplay(mover.phoneNumber)
          : '-';
  ...
  return (
    <div>
      ...
      <AdminDetailModal
        ...
        data={
          selectedMover && selectedRecordType
            ? selectedMover[selectedRecordType]
            : null
        }
      ...
      >
        <div className="space-y-12">
          <p className="text-gray-900">
            ...
            <strong>{selectedMover?.name}</strong>?
          </p>
