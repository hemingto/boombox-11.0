...
export interface CitiesSectionProps {
  ...
  /**
   * Slugs of published location pages.
   * When provided, only cities with a matching published page are displayed.
   */
...
export const CitiesSection: React.FC<CitiesSectionProps> = ({
  ...
  const cities = publishedSlugs
    ? bayAreaCities.filter(c => {
        const slug = c.link.replace('/locations/', '');
  ...
  return (
    <section
      className={cn('mt-12 sm:mt-24 lg:px-12 px-6 sm:mb-24 mb-12', className)}
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row w-full justify-between items-left sm:items-center mb-10">
        ...
        <nav className="flex mt-4 sm:mt-0 gap-1" aria-label="Cities pagination">
      ...
      {/* Cities Grid */}
      <div
        className={cn('grid gap-4 lg:gap-6 text-nowrap', gridColsClass)}
        role="list"
