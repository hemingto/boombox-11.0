...
function GetQuoteFormContent() {
  ...
  const handlePlanChange = useCallback(
    (id: string, planName: string, description: string) => {
  ...
  const handleDateBookingCountChange = useCallback(
    (bookingCount: number | null) => {
      const isDIY = state.selectedPlanName === 'Do It Yourself Plan';
      actions.setGreenDateDiscount(isDIY && bookingCount === 0);
    },
    [state.selectedPlanName, actions]
  );
  ...
  const handleLaborSelect = useCallback(
    (id: string, price: string, title: string, onfleetTeamId?: string) => {
  ...
  const handleUnavailableLaborChange = useCallback(
    (hasError: boolean) => {
    ...
    if (state.scheduledDate && state.scheduledTimeSlot) {
      try {
        ...
        actions.setSubmitError(
          'Invalid date or time selected. Please try again.'
        );
        return;
      }
    }
  ...
  }, [state, actions, submitQuote, submissionError]);
    ...
    <>
      ...
      <div className="md:flex gap-x-8 lg:gap-x-16 mt-12 sm:mt-24 lg:px-16 px-4 justify-center mb-10 sm:mb-64 items-start">
        ...
        <div className="w-full basis-1/2">
          <div role="form" aria-label="Get Quote Form">
            {/* Step Rendering */}
            {currentStep === 1 && (
              <QuoteBuilder
                // Address fields
                ...
                // Storage term fields
                selectedStorageTerm={state.storageTerm}
                storageTermError={state.storageTermError}
                onStorageTermChange={actions.setStorageTerm}
                clearStorageTermError={actions.clearStorageTermError}
                // Storage unit fields
                storageUnitCount={state.storageUnitCount}
        ...
        {currentStep !== 5 && (
          ...
          >
            <MyQuote
              showSendQuoteEmail={currentStep >= 4}
              // Address and location
              address={state.address}
              zipCode={state.zipCode}
              coordinates={state.coordinates}
              // Storage term
              storageTerm={state.storageTerm}
              planType={state.planType}
              // Storage and plan
              storageUnitCount={state.storageUnitCount}
              ...
              // Green date discount
              hasGreenDateDiscount={state.hasGreenDateDiscount}
            />
          </aside>
        )}
      </div>
