...
export function FeedbackForm({
  ...
  } = useFeedbackForm({ appointmentId, drivers, invoiceTotal, storageUnits });
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
        ...
        {tipPaymentStatus === 'failed' && (
          ...
          >
            <p className="text-status-warning text-sm mb-2">
              Your feedback was submitted successfully, but there was an issue
              processing your tip payment.
            </p>
            <p className="text-status-warning text-sm">
              Please contact support at help@boomboxstorage.com if you&apos;d
              like to try again.
            </p>
          </div>
        )}
        ...
        {tipPaymentStatus === 'processing' && (
          <div
            className="bg-status-bg-info border border-status-info rounded-md p-4 mb-6 max-w-md"
            role="status"
          >
        ...
        {tipPaymentStatus === 'succeeded' && (
          <div
            className="bg-status-bg-success border border-status-success rounded-md p-4 mb-6 max-w-md"
            role="status"
          >
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-8 min-h-[60vh] mb-24">
      {/* Appointment Header */}
      <section
        className="flex items-center mb-4 border-b border-surface-tertiary pb-8"
      ...
      >
        <div className="w-24 h-24 bg-surface-tertiary rounded-md mr-4 overflow-hidden flex items-center justify-center">
          {(() => {
            ...
            } else if (movingPartnerImageSrc) {
              src = movingPartnerImageSrc;
        ...
        <div>
          ...
          <p className="text-text-secondary">{appointmentDate}</p>
          <p className="text-text-secondary">{appointmentType}</p>
        </div>
      </section>
      ...
      {drivers.length > 0 && (
        <section
          className="mb-8 border-b border-surface-tertiary pb-8"
          aria-labelledby="driver-ratings-heading"
        >
          <h3
            id="driver-ratings-heading"
                ...
                <div className="flex items-center">
                  <div className="w-12 h-12 bg-surface-tertiary rounded-md mr-3 overflow-hidden flex items-center justify-center">
                    {driver.cloudinaryFile && (
                      <Image
                        src={
                          driver.cloudinaryFile.startsWith('https://')
                            ? driver.cloudinaryFile
                  ...
                  <button
                    onClick={() =>
                      setDriverRatings(prev => ({
                        ...prev,
                        [driver.key]:
                          prev[driver.key] === 'thumbs_up' ? null : 'thumbs_up',
                      }))
                    }
      ...
      {/* Tip Selection */}
      <section
        className="border-b border-surface-tertiary pb-8 mb-8"
        aria-labelledby="tip-selection-heading"
      >
        <h3
          id="tip-selection-heading"
          className="text-xl font-semibold mb-1 text-text-primary"
        >
          Add a tip
        </h3>
        ...
        {isCustomTip && (
          <div className="mt-4 relative">
            ...
            <span
              className="absolute text-lg left-4 top-1/2 -translate-y-1/2 text-text-primary"
              aria-hidden="true"
            >
              $
            </span>
      ...
      >
        ...
        <h3 id="comment-heading" className="sr-only">
          Comments
        </h3>
        <textarea
          id="feedback-comment"
      ...
      {/* Padlock Combo Section */}
      {storageUnits && storageUnits.length > 0 && (
        <section className="mb-16" aria-labelledby="padlock-combo-heading">
          <div className="bg-amber-100 rounded-md p-4 mb-4">
            <h3
              id="padlock-combo-heading"
              className="text-lg text-amber-500 font-semibold mb-2"
            >
              Never forget your padlock combo!
            </h3>
            <p className="text-sm text-amber-500">
              enter your padlock combo below to save it to your account
            </p>
          </div>
          <div>
            <div className="space-y-4">
              {storageUnits.map(unit => (
                <Input
                  key={unit.usageId}
                  id={`padlock-combo-${unit.usageId}`}
                  label={`Boombox ${unit.serialNumber}`}
                  value={padlockCombos[unit.usageId] || ''}
                  onChange={e => {
                    setPadlockCombos(prev => ({
                      ...prev,
                      [unit.usageId]: e.target.value,
                    }));
                  }}
      ...
      {errorMessage && (
        <div
          className="mb-6 p-3 bg-status-bg-error border border-status-error rounded-md"
          role="alert"
        >
