  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
        ...
        {tipPaymentStatus === 'failed' && (
          <div className="bg-status-bg-warning border border-status-warning rounded-md p-4 mb-6 max-w-md" role="alert">
            <p className="text-status-text-warning text-sm mb-2">
        ...
        {tipPaymentStatus === 'processing' && (
          <div className="bg-status-bg-info border border-status-info rounded-md p-4 mb-6 max-w-md" role="status">
            <p className="text-status-text-info text-sm">
        ...
        {tipPaymentStatus === 'succeeded' && (
          <div className="bg-status-bg-success border border-status-success rounded-md p-4 mb-6 max-w-md" role="status">
            <p className="text-status-text-success text-sm">
    ...
    <main className="max-w-2xl mx-auto px-4 py-8 min-h-[60vh] mb-24">
                  ...
                  <button
                    ...
                    className={`p-3 rounded-full transition-colors ${
                      driverRatings[driver.key] === 'thumbs_up'
                        ? 'bg-status-bg-success border-status-success text-status-text-success'
                  ...
                  <button
                    ...
                    className={`p-3 rounded-full transition-colors ${
                      driverRatings[driver.key] === 'thumbs_down'
                        ? 'bg-status-bg-error text-status-text-error'
                        : 'bg-surface-tertiary text-text-secondary hover:bg-surface-disabled'
                    }`}
      ...
      {errorMessage && (
        <div className="mb-6 p-3 bg-status-bg-error border border-status-error rounded-md" role="alert">
          <p className="text-status-text-error text-sm">{errorMessage}</p>
        </div>
      )}
      ...
      <button
        ...
        className={`w-full font-semibold py-3 rounded-md transition-colors ${
          submitting 
            ? 'bg-primary-hover text-white cursor-not-allowed' 
            : 'btn-primary'
        }`}
        type="button"
      ...
      >
        {submitting ? (
          <span className="flex items-center justify-center">
            ...
            <span id="submit-status">Processing...</span>
          </span>
        ) : (
          'Submit'
        )}
