...
function AccessStorageForm() {
  ...
  const { formState, errors, isSubmitting, setError, updateFormState } =
    useAccessStorageFormState();
  const { currentStep, goToStep, goToNextStep } =
    useAccessStorageNavigation_Context();
    ...
    switch (currentStep) {
        ...
        return (
          <Scheduler
            ...
            onDateTimeSelected={(date: Date, timeSlot: string) => {
              ...
              form.setValue('scheduledTimeSlot', timeSlot, {
                shouldValidate: true,
              });
      ...
      case AccessStorageStep.LABOR_SELECTION:
        if (formState.planType === 'Do It Yourself Plan') {
          return (
            <div className="text-text-secondary">Loading confirmation...</div>
          );
        }
        return (
          <ChooseLabor
            ...
            onLaborSelect={(
              ...
              const thirdPartyMovingPartnerId = id.startsWith('thirdParty-')
                ? parseInt(id.replace('thirdParty-', ''), 10)
                : null;

              // Update React Hook Form (source of truth for validation)
              form.setValue('selectedLabor', selectedLabor, {
                shouldValidate: true,
              });
              form.setValue('planType', planType, { shouldValidate: true });
              form.setValue('selectedPlanName', title, {
                shouldValidate: false,
              });
              form.setValue('loadingHelpPrice', formattedPrice, {
                shouldValidate: false,
              });
              form.setValue('parsedLoadingHelpPrice', parsedPrice, {
                shouldValidate: false,
              });
              form.setValue('movingPartnerId', movingPartnerId, {
                shouldValidate: false,
              });
              form.setValue(
              ...
              updateFormState({
                ...
                planType,
                selectedPlan:
                  id === 'Do It Yourself Plan' ? 'option1' : 'option2',
                movingPartnerId,
                thirdPartyMovingPartnerId,
              });
            }}
            ...
            onMovingPartnerSelect={(partnerId: number | null) => {
              form.setValue('movingPartnerId', partnerId, {
                shouldValidate: false,
              });
            ...
            onUnavailableLaborChange={(hasError: boolean, message?: string) => {
              setError(
                ...
                hasError
                  ? message || 'Mover unavailable. Choose another.'
                  : null
              );
            }}
  ...
  const myQuoteTitle =
    formState.deliveryReason === 'End storage term'
      ? 'End storage term'
      : 'Storage access';
    ...
    () => ({
      ...
      isButtonDisabled:
        ...
        (currentStep === AccessStorageStep.LABOR_SELECTION &&
          form.watch('planType') !== 'Do It Yourself Plan' &&
          !form.watch('selectedLabor')),
    }),
    [
      myQuoteTitle,
      formState.address,
  ...
  return (
    <div className="md:flex gap-x-8 lg:gap-x-16 mt-12 sm:mt-24 lg:px-16 px-6 justify-center mb-10 sm:mb-64 items-start">
      ...
      <div className="basis-1/2 md:mr-auto sticky top-5 max-w-md sm:h-[500px]">
        ...
        {errors.submitError &&
          currentStep === AccessStorageStep.CONFIRMATION && (
            <div
              className="text-status-error text-sm mb-2 text-center md:text-left"
              role="alert"
              aria-live="polite"
