...
function AccessStorageStep1() {
  ...
  const handleDeliveryReasonChange = useCallback(
    (value: string | null) => {
      ...
      if (
        deliveryReason === DeliveryReason.END_STORAGE_TERM &&
  ...
  return (
    <div className="w-full basis-1/2" role="main" aria-labelledby="step1-title">
      <div className="max-w-lg mx-auto md:mx-0 md:ml-auto">
        ...
        <section
          className="form-group mb-4 sm:mb-8"
          aria-labelledby="delivery-purpose-heading"
        >
          <YesOrNoRadio
            ...
            errorMessage={
              deliveryReasonField.error || 'Please make a selection'
            }
            name="deliveryReason"
        ...
        <section
          ...
          aria-labelledby="storage-units-heading"
        >
          ...
          {storageUnitsError && (
            <div className="bg-status-bg-error p-4 rounded-md">
              <p className="form-error mb-4" role="alert" aria-live="polite">
                Failed to load storage units. Please refresh the page and try
                again.
              </p>
            </div>
          )}
          ...
          {storageUnitsLoading ? (
            <div
              className="space-y-4"
              role="status"
              aria-label="Loading storage units"
            >
              <div
                className="skeleton skeleton-text w-14 h-4"
                aria-hidden="true"
              ></div>
            ...
            <>
              ...
              />
              {storageUnits.length > 0 &&
                (deliveryReasonField.value ===
                DeliveryReason.END_STORAGE_TERM ? (
                  <div
                    id="storage-units-help-end-term"
                    className="mt-4 p-3 sm:mb-4 mb-2 bg-surface-primary border border-border rounded-md max-w-fit"
                  >
                    <p className="text-xs">
                      All units are automatically selected when ending your
                      storage term
                    </p>
                  </div>
                ) : (
                  <div
                    id="storage-units-help"
                    className="mt-4 p-3 sm:mb-4 mb-2 bg-surface-primary border border-border rounded-md max-w-fit"
                  >
                    <p className="text-xs">
