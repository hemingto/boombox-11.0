...
export function ChooseLabor({
 ...
 return (
  <div className="w-full basis-1/2 mb-96 sm:mb-60 sm:h-[500px]">
   <div className="max-w-lg mx-auto md:mx-0 md:ml-auto">
    ...
    <p className="mb-3 text-text-primary">Choose from our list of moving partners</p>  
    ...
    {movingPartners.length > 2 && (
     <div className="mb-4">
      <div className="relative">
       <div
        className={`relative w-fit rounded-full p-2 cursor-pointer ${
         isSortDropdownOpen
          ? "ring-2 ring-slate-100 bg-surface-primary"
          : "ring-2 ring-slate-100 bg-surface-tertiary"
        }`}
    ...
    {!isLoading && movingPartners.length === 0 ? (
     <>
      {!laborError && !selectedWeblink && (
       <div className="bg-status-bg-warning p-3 mb-4 border-2 border-border-warning rounded-md max-w-fit">
      ...
      {selectedWeblink && selectedLabor?.title !== "Do It Yourself Plan" && (
       <div className="bg-status-bg-info p-3 mb-4 border-2 border-border-info rounded-md max-w-fit">
        ...
        <a
         ...
         className="block"
        >
         <div className="flex rounded-full px-3 py-2 bg-cyan-500 mt-3 hover:bg-cyan-400 active:bg-cyan-500 w-fit">
      ...
      <ThirdPartyLaborList
       ...
       onLaborSelect={(id, price, title) => {
        onLaborSelect(id, price, title);
        onMovingPartnerSelect(null); // Clear moving partner for third party
       }}
      ...
      <DoItYourselfCard
       ...
       onChange={(id, price, description) => {
        ...
        onMovingPartnerSelect(null); // Clear moving partner for DIY
       }}
       hasError={!!laborError}
       onClearError={clearLaborError}
       onPlanTypeChange={onPlanTypeChange} 
     ...
     currentItems.map((partner) => (
      <LaborRadioCard
       ...
       onChange={() => {
        ...
        onMovingPartnerSelect(partner.id); // Set the moving partner ID
    ...
    {movingPartners.length > 5 && (
     <div className="flex justify-center items-center mt-8 space-x-4">
      <button
       ...
       className="p-2 rounded-md text-text-secondary hover:bg-surface-tertiary disabled:opacity-50 disabled:cursor-not-allowed"
