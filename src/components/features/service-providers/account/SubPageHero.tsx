...
interface SubPageHeroProps {
  ...
  description?: string;
  /** Type of user account (driver, mover/moving partner, or hauler) */
...
export function SubPageHero({
  ...
  const backLabel =
    userType === 'driver'
      ? 'Back to Driver Account'
      : userType === 'hauler'
        ? 'Back to Hauler Account'
        : 'Back to Mover Account';
  ...
  return (
    ...
    >
      ...
      {description && <p className="mt-4 text-text-primary">{description}</p>}
    </section>
  );
}
