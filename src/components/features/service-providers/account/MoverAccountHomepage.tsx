...
export function MoverAccountHomepage({
  ...
  const [isMovingPartnerStatusLoading, setIsMovingPartnerStatusLoading] =
    useState(true);
  ...
  const shouldDisableButtons =
    !isApproved || moverStatus.toUpperCase() !== 'ACTIVE';
  ...
  // Fetch approval status for drivers, movers, and haulers
  useEffect(() => {
    if (!userId) {
    ...
    if (userType === 'mover' || userType === 'hauler') {
      ...
      fetch(profileEndpoint)
        ...
        .catch(err => console.error(`Error fetching ${userType} data:`, err))
  ...
  useEffect(() => {
    if (userType === 'driver' && userId) {
      ...
      fetch(`/api/drivers/${userId}/moving-partner-status`)
        ...
        .catch(err =>
          console.error('Error fetching moving partner status:', err)
        )
        ...
        {isDataLoading ? (
          ...
          <>
            ...
            {((userType === 'driver' &&
              ...
              <MoverAccountOptions
                icon={
                  <CalendarDaysIcon className="h-8 w-8 text-text-primary" />
                }
                title="Work Schedule"
            ...
            {/* Account Information - Mover/Hauler version */}
            ...
            {/* Driver Information - Mover/Hauler only, disabled until approved */}
            {(userType === 'mover' || userType === 'hauler') && (
              <MoverAccountOptions
                icon={
                  <IdentificationIcon className="h-8 w-8 text-text-primary" />
                }
