...
const DriversLicenseImages: React.FC<DriversLicenseImagesProps> = ({
  ...
  const [photoToDelete, setPhotoToDelete] = useState<'front' | 'back' | null>(
    null
  );
  ...
  const handleDeletePhoto = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/drivers/${userId}/remove-license-photos`,
        {
          method: 'DELETE',
          headers: {
  ...
  return (
    <>
      <h2 className="text-2xl text-text-primary mt-4 mb-8">
        Driver&apos;s License
      </h2>
      ...
      {!driverLicenseFront && !driverLicenseBack && !isLoadingData && (
        ...
        >
          <p className="text-status-warning text-sm">
            To activate your driver account upload a front and back photo of
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="w-full">
          ...
          {isLoadingData ? (
            <div className="bg-surface-tertiary rounded-md p-4 aspect-video flex items-center justify-center">
              <CreditCardIcon
                className="w-16 h-16 text-text-secondary mb-1"
                aria-hidden="true"
              />
            </div>
          ) : driverLicenseBack ? (
