...
export const AppointmentDetailsPopup: React.FC<
  AppointmentDetailsPopupProps
> = ({ isOpen, onClose, appointment, userType }) => {
  if (!appointment) return null;

  const isPackingSupplyRoute =
  ...
  if (isPackingSupplyRoute && appointment.estimatedDurationMinutes) {
    endTime.setMinutes(
      endTime.getMinutes() + appointment.estimatedDurationMinutes
    );
  } else {
        ...
        <div>
          ...
          >
            {isPackingSupplyRoute
              ? `Route ${appointment.routeId}`
              : `${appointment.appointmentType} appt`}
          </h2>
          ...
          {isPackingSupplyRoute && (
            <p className="text-sm text-zinc-400 mt-1">
              {appointment.totalStops} stops • {appointment.completedStops || 0}{' '}
              completed
            </p>
          )}
        </div>
      ...
      <div className="space-y-6 mt-6 mb-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-status-error text-sm">
                      please make sure to assign a driver by clicking the red
                      driver unassigned button.
                    </p>
                  </div>
                </div>
              </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-b border-zinc-200 pb-6">
            ...
            {isPackingSupplyRoute ? (
              <div className="space-y-1">
                <p className="text-text-primary">
                  Status: {appointment.routeStatus?.replace('_', ' ')}
                </p>
                {appointment.estimatedPayout && (
                  <p className="text-text-primary">
                    Payout: ${appointment.estimatedPayout}
                  </p>
                )}
              </div>
            ) : (
          ...
          <div>
            ...
            {isPackingSupplyRoute ? (
              <div className="space-y-1">
                <p className="text-text-primary">
                  {appointment.totalStops} delivery stops
                </p>
                {appointment.estimatedMiles && (
                  <p className="text-text-primary">
                    {appointment.estimatedMiles} miles
                  </p>
                )}
              </div>
            ) : (
            ...
            {isPackingSupplyRoute ? (
              <>
                ...
                {appointment.estimatedDurationMinutes && (
                  <p>
                    Est. Duration:{' '}
                    {Math.floor(appointment.estimatedDurationMinutes / 60)}h{' '}
                    {appointment.estimatedDurationMinutes % 60}m
                  </p>
                )}
                {appointment.payoutStatus && (
                  <p>
                    Payout Status: {appointment.payoutStatus.replace('_', ' ')}
                  </p>
                )}
              </>
              ...
              <>
                <p>
                  {appointment.requestedStorageUnits?.length || 0} Requested
                  Unit
                  {(appointment.requestedStorageUnits?.length || 0) !== 1
                    ? 's'
                    : ''}
                </p>
        ...
        {isPackingSupplyRoute ? (
          ...
          <section aria-labelledby="job-description-heading">
            ...
            <div className="space-y-1 text-zinc-600">
              <p>
                {appointment.description || 'No added customer description'}
              </p>
              {appointment.additionalInformation?.itemsOver100lbs !==
                undefined && (
                <p>
                  {appointment.additionalInformation.itemsOver100lbs === false
                    ? 'No Items over 100 lbs'
                    : 'Has items over 100 lbs'}
                </p>
              )}
