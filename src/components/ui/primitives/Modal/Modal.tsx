...
const Modal: React.FC<ModalProps> = ({
 ...
 useEffect(() => {
  ...
  if (open) {
   ...
   // Only focus on first open, not on subsequent prop changes
   ...
   isFirstRender.current = true; // Reset for next open
  }
  ...
  <div className="fixed inset-0 z-50">
   ...
   <div
    ...
    className={cn(
     'modal-content',
     'w-full',
     effectiveSize === 'full' ? 'h-[90vh]' : 'max-h-[90vh]',
     'flex flex-col',
   ...
   >
    ...
    {effectiveShowCloseButton && (
     <button
      onClick={onClose}
      className={cn(
       "absolute top-4 right-4 p-2 rounded-full hover:bg-surface-tertiary active:bg-surface-disabled z-10",
    ...
    {title && (
     <div className="mb-4 flex-shrink-0">
      <h2
       id="modal-title"
       className={cn(
        "text-xl font-semibold text-text-primary mt-4",
    ...
    {/* Content */}
    <div className={cn(
     effectiveSize === 'full' ? 'flex-1 min-h-0' : 'flex-1 overflow-y-auto',
     'p-1', // Add horizontal padding to prevent focus ring cutoff
