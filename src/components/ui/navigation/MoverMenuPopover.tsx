...
export const MoverMenuPopover: React.FC<MoverMenuPopoverProps> = ({ className, theme = 'dark', userType, userId }) => {
  ...
  const handleLogout = async () => {
    try {
      ...
      console.error('[MoverMenuPopover] Logout failed:', error);
          ...
          {menuOptions.map((option) => (
            <li key={option.name} role="listitem">
              <Link 
                ...
                className="group flex h-10 space-x-4 items-center p-4 text-nowrap text-sm cursor-pointer text-text-primary hover:bg-surface-tertiary active:bg-surface-disabled group-hover:text-text-primary focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset"
                onClick={closeMenu}
              >
