  ...
  return (
    <div className="relative inline-block text-left" ref={popoverRef}>
      <Button
        ...
        aria-haspopup="menu"
        icon={<ChevronDownIcon className="h-4 w-4 stroke-2" />}
        iconPosition="right"
        className="text-text-inverse hover:bg-primary-hover active:bg-primary-active transition-colors"
      >
        Locations
      </Button>
      ...
      >
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1" role="none">
          {cities.map((city, index) => (
            <Link 
              ...
              className={`py-2.5 px-3 text-nowrap text-sm underline-offset-4 hover:underline hover:bg-surface-tertiary rounded transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:bg-surface-tertiary ${
                focusedIndex === index ? 'bg-surface-tertiary' : ''
              }`}
        ...
        <div 
          className={`group relative leading-6 p-4 border-t border-border cursor-pointer hover:bg-surface-tertiary active:bg-surface-disabled transition-colors ${
            focusedIndex === cities.length ? 'bg-surface-tertiary' : ''
          }`}
          role="none"
        >
          <Link 
            className="flex justify-between grow items-center focus:outline-none focus:ring-2 focus:ring-primary rounded" 
            href="/locations"
            role="menuitem"
            tabIndex={isOpen ? 0 : -1}
            ...
            onClick={closeMenu}
          >
            <p className="pl-2 text-text-primary">Check our full list of locations</p>
            <ChevronRightIcon className="h-5 w-5 text-text-primary mr-2" aria-hidden="true"/>
          </Link>
        </div>
      </div>
      </div>
