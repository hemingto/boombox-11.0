...
export async function createOnfleetTasksWithDatabaseSave(payload: any) {
  ...
  console.log(
    `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - createOnfleetTasksWithDatabaseSave called with payload:`,
  ...
  const {
    calculateTaskTiming,
    buildTaskMetadata,
    buildTaskCustomFields,
    buildTaskNotes,
  } = await import('@/lib/services/onfleet/taskBuilder');
  ...
  // Fetch jobCode and moving partner name from the appointment record
  const appointmentRecord = await prisma.appointment.findUnique({
    where: { id: parseInt(String(payload.appointmentId), 10) },
    select: { jobCode: true, movingPartner: { select: { name: true } } },
  });
  const jobCode = payload.jobCode || appointmentRecord?.jobCode || '';
  const movingPartnerName =
    payload.movingPartnerName ||
    appointmentRecord?.movingPartner?.name ||
    'Moving Partner';
  ...
  const isAdditionalStorageAppointment =
    payload.appointmentType === 'Additional Storage';
  ...
  existingTasks.forEach(task => {
    if (
      task.stepNumber &&
      (task.stepNumber === 1 || task.stepNumber === 2 || task.stepNumber === 3)
    ) {
  ...
  console.log(
    `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - Starting unit number: ${startingUnitNumber}, unitCount: ${unitCount}`
  );
  ...
  storageUnits.forEach(unit => {
    storageUnitNumbersMap[unit.id] =
      unit.storageUnitNumber || `Unit #${unit.id}`;
  });
  ...
  console.log(
    `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - Calculating cost breakdowns for address: ${payload.address}`
  );
  ...
  try {
    ...
    const step1Breakdown = await calculateStepCostBreakdown(
      1,
      'boombox_driver',
      customerAddressForCalc,
      appointmentTypeForCalc
    );
    const step2Breakdown = await calculateStepCostBreakdown(
      2,
      'boombox_driver',
      customerAddressForCalc,
      appointmentTypeForCalc
    );
    const step3Breakdown = await calculateStepCostBreakdown(
      3,
      'boombox_driver',
      customerAddressForCalc,
      appointmentTypeForCalc
    ...
    console.log(
      `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - Cost breakdowns calculated:`,
      {
        step1: {
          total: step1Breakdown.totalCost,
          fixed: step1Breakdown.fixedFeePay,
        },
        step2: {
          total: step2Breakdown.totalCost,
          mileage: step2Breakdown.mileagePay,
          drive: step2Breakdown.driveTimePay,
          service: step2Breakdown.serviceTimePay,
        },
        step3: {
          total: step3Breakdown.totalCost,
          mileage: step3Breakdown.mileagePay,
          drive: step3Breakdown.driveTimePay,
        },
    ...
    console.error(
      `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - Error calculating cost breakdowns:`,
      costError
    );
  ...
  try {
    for (let i = 0; i < unitCount; i++) {
      ...
      console.log(
        `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - Iteration ${i} for unit ${currentUnitIterationNumber}`
      );

      // Check if tasks for this specific unit number already exist (idempotency)
      const tasksForThisUnitNumber = existingTasks.filter(
        task => task.unitNumber === currentUnitIterationNumber
      );
      const step1ExistingTask = tasksForThisUnitNumber.find(
        task => task.stepNumber === 1 && task.taskId
      );
      const step2ExistingTask = tasksForThisUnitNumber.find(
        task => task.stepNumber === 2 && task.taskId
      );
      const step3ExistingTask = tasksForThisUnitNumber.find(
        task => task.stepNumber === 3 && task.taskId
      );

      if (step1ExistingTask && step2ExistingTask && step3ExistingTask) {
        console.log(
          `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - Tasks for unit ${currentUnitIterationNumber} already exist. Skipping.`
        );
        ...
        console.warn(
          `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - Partial tasks found for unit ${currentUnitIterationNumber}. Proceeding with creation.`
        );
      }
      ...
      const specificUnitId =
        payload.storageUnitIds && payload.storageUnitIds[i]
          ? payload.storageUnitIds[i]
          : null;

      const specificStorageUnitNumberLabel =
        specificUnitId && storageUnitNumbersMap[specificUnitId]
          ? storageUnitNumbersMap[specificUnitId]
          : specificUnitId
            ? `DB Unit ID #${specificUnitId}`
            : `Calculated Unit #${currentUnitIterationNumber}`;

      console.log(
        `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - Creating tasks for unit: ${specificStorageUnitNumberLabel}`
      );

      // Get all unit numbers as a comma-separated string
      const allUnitNumbers = storageUnitIds
        .map((id: number) => storageUnitNumbersMap[id] || id.toString())
        .join(', ');

      // Determine team and plan type
      const isDIY =
        payload.selectedPlanName === 'Do It Yourself Plan' ||
        !payload.selectedLabor;
      const teamId =
        isDIY || currentUnitIterationNumber > 1
          ? process.env.BOOMBOX_DELIVERY_NETWORK_TEAM_ID
          : payload.selectedLabor?.onfleetTeamId ||
            process.env.BOOMBOX_DELIVERY_NETWORK_TEAM_ID;
      ...
      const notes = buildTaskNotes({
        ...
        jobCode,
        isDIY,
        isFirstUnit: currentUnitIterationNumber === 1,
        totalUnits: unitCount,
        currentUnitNumber: currentUnitIterationNumber,
      ...
      const metadataConfig = {
        ...
        storageUnitCount: isAccessAppointment
      ...
      if (!pickupTaskResponse.ok) {
        ...
        throw new Error(
          `Failed to create Onfleet pickup task: ${JSON.stringify(errorData)}`
        );
      }
      ...
      if (!customerTaskResponse.ok) {
        ...
        throw new Error(
          `Failed to create Onfleet customer task: ${JSON.stringify(errorData)}`
        );
      }
      ...
      if (!returnTaskResponse.ok) {
        ...
        throw new Error(
          `Failed to create Onfleet return task: ${JSON.stringify(errorData)}`
        );
      }
      ...
      createdPrismaTasks.push(
        {
          ...
          estimatedDriveTimeMinutes:
            step1Cost?.estimatedDriveTimeMinutes || null,
        ...
        },
        {
          ...
          estimatedDriveTimeMinutes:
            step2Cost?.estimatedDriveTimeMinutes || null,
        ...
        },
        {
          ...
          estimatedDriveTimeMinutes:
            step3Cost?.estimatedDriveTimeMinutes || null,
      ...
      console.log(
        `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - Successfully created 3 Onfleet tasks for unit ${currentUnitIterationNumber}.`
      );
    }
    ...
    if (createdPrismaTasks.length > 0) {
      ...
      console.log(
        `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - Saved ${createdPrismaTasks.length} tasks to database`
      );
    }
    ...
    const totalEstimatedCost =
      stepCostBreakdowns.reduce(
        (sum, step) => sum + (step?.totalCost || 0),
        0
      ) * unitCount;
    if (totalEstimatedCost > 0) {
      try {
        ...
        console.log(
          `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - Saved totalEstimatedCost: $${totalEstimatedCost.toFixed(2)} to appointment ${payload.appointmentId}`
        );
      } catch (updateError) {
        console.error(
          `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - Error saving totalEstimatedCost:`,
          updateError
        );
      }
    }
    ...
    console.log(
      `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - Cost breakdowns saved with tasks`
    );

    console.log(
      `ONFLEET_SERVICE: DEBUG [${uniqueCallId}] - createOnfleetTasksWithDatabaseSave finished successfully`
    );
    return {
      taskIds: allTaskIds,
