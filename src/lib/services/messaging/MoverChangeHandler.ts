...
export class MoverChangeHandler {
   ...
   */
  async handleResponse(phoneNumber: string, action: 'accept' | 'diy'): Promise<MessageRouteResult> {
    try {
      ...
      if (!pendingAppointment) {
        await MessageService.sendSms(phoneNumber, noPendingMoverChangeTemplate, {});
        return { success: false, error: 'No pending mover change found' };
      }

      // Parse appointment description to validate mover change status
      const appointmentDescription = parseAppointmentDescription(pendingAppointment.description);
      ...
      if (!moverChangeRequest || moverChangeRequest.status !== 'pending') {
        await MessageService.sendSms(phoneNumber, moverChangeAlreadyProcessedTemplate, {});
      ...
      const response = await fetch(`${config.app.url}/api/orders/mover-change-response`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        await MessageService.sendSms(phoneNumber, moverChangeProcessingErrorTemplate, {});
