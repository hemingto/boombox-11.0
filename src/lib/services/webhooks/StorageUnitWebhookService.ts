...
export class StorageUnitWebhookService {
  ...
  static async handle(
    ...
    try {
      ...
      if (!taskDetails) {
        console.error(
          '[StorageUnitWebhook] ERROR: No task details in webhook data'
        );
      ...
      console.log(
        `[StorageUnitWebhook] Task metadata:`,
        JSON.stringify(metadata, null, 2)
      );

      // Find the OnfleetTask to get the unitNumber
      console.log(
        `[StorageUnitWebhook] Looking up OnfleetTask by shortId: ${taskDetails.shortId}`
      );
      ...
      if (onfleetTask) {
        ...
        console.log(
          `[StorageUnitWebhook] WARNING: OnfleetTask NOT FOUND for shortId: ${taskDetails.shortId}`
        );
      }

      // Fall back to the database stepNumber when metadata is missing
      const resolvedStep =
        step ??
        (onfleetTask?.stepNumber != null
          ? String(onfleetTask.stepNumber)
          : null);
      if (!step && resolvedStep) {
        console.log(
          `[StorageUnitWebhook] step metadata missing, falling back to DB stepNumber: ${resolvedStep}`
        );
      }

      const stepNumber = Number(resolvedStep);
      console.log(
        `[StorageUnitWebhook] Step number (parsed): ${stepNumber}, is valid step: ${[1, 2, 3, 4].includes(stepNumber)}`
      );

      if (triggerName === 'taskStarted' && [1, 2, 3, 4].includes(stepNumber)) {
        if (onfleetTask) {
          console.log(
            `[StorageUnitWebhook] Updating webhook time for taskStarted, step ${resolvedStep}`
          );
          await updateOnfleetTaskWebhookTime(
            taskDetails.shortId,
            time.toString()
          );
          ...
          console.log(
            `[StorageUnitWebhook] Skipping webhook time update - OnfleetTask not found in database for shortId: ${taskDetails.shortId}`
          );
        }
      }
      ...
      if (
        ...
        [1, 2, 3, 4].includes(stepNumber)
      ) {
        if (onfleetTask) {
          console.log(
            `[StorageUnitWebhook] Saving completion photo for step ${resolvedStep}`
          );
          await updateTaskCompletionPhoto(taskDetails);
          console.log(`[StorageUnitWebhook] Completion photo saved`);
        } else {
          console.log(
            `[StorageUnitWebhook] Skipping completion photo save - OnfleetTask not found in database for shortId: ${taskDetails.shortId}`
          );
        }
      }
      ...
      if (
        ...
        if (allPhotoUrls.length > 0) {
          ...
          console.log(
            `[StorageUnitWebhook] Updating StorageUnitUsage photos for storageUnitId: ${onfleetTask.storageUnitId}, unitNumber: ${onfleetTask.unitNumber}`
          );
          try {
            ...
            console.log(
              `[StorageUnitWebhook] StorageUnitUsage photos updated successfully`
            );
          } catch (error) {
            console.error(
              `[StorageUnitWebhook] Error updating StorageUnitUsage photos:`,
              error
            );
          }
        } else {
          console.log(
            `[StorageUnitWebhook] No completion photos found in webhook for storageUnitId: ${onfleetTask.storageUnitId}`
          );
        }
      }
      ...
      if (
        ...
        console.log(
          `[StorageUnitWebhook] Calculating actual costs for step ${resolvedStep}`
        );
        try {
          ...
          console.log(
            '[StorageUnitWebhook] Actual cost updated successfully for task:',
            taskDetails.shortId
          );
        } catch (error) {
          console.error(
            '[StorageUnitWebhook] Error updating actual cost for task:',
            taskDetails.shortId,
            error
          );
      ...
      console.log(
        `[StorageUnitWebhook] Unit number check: ${onfleetTask?.unitNumber} === 1 ? ${onfleetTask?.unitNumber === 1}`
      );

      if (onfleetTask?.unitNumber === 1) {
        console.log(
          `[StorageUnitWebhook] >>> Processing main unit (unit 1) webhook`
        );
        await this.processMainUnitWebhook(webhookData, resolvedStep);
        console.log(`[StorageUnitWebhook] Main unit processing complete`);
      } else {
        console.log(
          `[StorageUnitWebhook] Skipping billing/notifications - not unit 1 (unitNumber: ${onfleetTask?.unitNumber})`
        );
      }
      ...
      return {
        success: true,
        message: 'Storage unit webhook processed successfully',
      };
    } catch (error) {
      console.error('[StorageUnitWebhook] ERROR during processing:', error);
      console.error(
        '[StorageUnitWebhook] Error stack:',
  ...
  private static async processMainUnitWebhook(
    ...
    console.log(
      `[StorageUnitWebhook:MainUnit] Processing - trigger: ${triggerName}, step: ${step}`
    );

    if (!step) {
      console.log(
        '[StorageUnitWebhook:MainUnit] WARNING: No step found in metadata, skipping step-specific processing'
      );
      return;
    }

    console.log(
      `[StorageUnitWebhook:MainUnit] Routing to step ${step} handler for trigger ${triggerName}`
    );
    ...
    switch (step) {
      case '1':
        console.log(
          `[StorageUnitWebhook:MainUnit] Step 1 - checking trigger: ${triggerName}`
        );
        if (triggerName === 'taskStarted') {
          console.log(
            '[StorageUnitWebhook:MainUnit] >>> Calling StepOneHandler.handleTaskStarted()'
          );
          await StepOneHandler.handleTaskStarted(webhookData);
          console.log(
            '[StorageUnitWebhook:MainUnit] StepOneHandler.handleTaskStarted() completed'
          );
        } else {
          console.log(
            `[StorageUnitWebhook:MainUnit] Step 1 - trigger ${triggerName} not handled`
          );
        }
        break;
      ...
      case '2':
        console.log(
          `[StorageUnitWebhook:MainUnit] Step 2 - checking trigger: ${triggerName}`
        );
        if (triggerName === 'taskStarted') {
          console.log(
            '[StorageUnitWebhook:MainUnit] >>> Calling StepTwoHandler.handleTaskStarted()'
          );
          await StepTwoHandler.handleTaskStarted(webhookData);
          console.log(
            '[StorageUnitWebhook:MainUnit] StepTwoHandler.handleTaskStarted() completed'
          );
        } else if (triggerName === 'taskArrival') {
          console.log(
            '[StorageUnitWebhook:MainUnit] >>> Calling StepTwoHandler.handleTaskArrival()'
          );
          await StepTwoHandler.handleTaskArrival(webhookData);
          console.log(
            '[StorageUnitWebhook:MainUnit] StepTwoHandler.handleTaskArrival() completed'
          );
        } else if (triggerName === 'taskCompleted') {
          console.log(
            '[StorageUnitWebhook:MainUnit] >>> Calling StepTwoHandler.handleTaskCompleted()'
          );
          await StepTwoHandler.handleTaskCompleted(webhookData);
          console.log(
            '[StorageUnitWebhook:MainUnit] StepTwoHandler.handleTaskCompleted() completed'
          );
        } else {
          console.log(
            `[StorageUnitWebhook:MainUnit] Step 2 - trigger ${triggerName} not handled`
          );
        }
        break;
      ...
      case '3':
        console.log(
          `[StorageUnitWebhook:MainUnit] Step 3 - checking trigger: ${triggerName}`
        );
        if (triggerName === 'taskCompleted') {
          console.log(
            '[StorageUnitWebhook:MainUnit] >>> Calling StepThreeHandler.handleTaskCompleted()'
          );
          await StepThreeHandler.handleTaskCompleted(webhookData);
          console.log(
            '[StorageUnitWebhook:MainUnit] StepThreeHandler.handleTaskCompleted() completed'
          );
        } else {
          console.log(
            `[StorageUnitWebhook:MainUnit] Step 3 - trigger ${triggerName} not handled`
          );
        }
        break;
      ...
      case '4':
        console.log(
          `[StorageUnitWebhook:MainUnit] Step 4 - checking trigger: ${triggerName}`
        );
        if (triggerName === 'taskCompleted') {
          console.log(
            '[StorageUnitWebhook:MainUnit] >>> Calling StepFourHandler.handleTaskCompleted()'
          );
          await StepFourHandler.handleTaskCompleted(webhookData);
          console.log(
            '[StorageUnitWebhook:MainUnit] StepFourHandler.handleTaskCompleted() completed'
          );
        } else {
          console.log(
            `[StorageUnitWebhook:MainUnit] Step 4 - trigger ${triggerName} not handled`
          );
        }
        break;
      ...
      default:
        console.log(
          `[StorageUnitWebhook:MainUnit] Unhandled step: ${step} for trigger: ${triggerName}`
        );
    }
