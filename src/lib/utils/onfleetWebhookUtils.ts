 ...
 */
export function extractDeliveryPhotoUrl(
  ...
  if (
    completionDetails.photoUploadIds &&
    completionDetails.photoUploadIds.length > 0
  ) {
  ...
  if (
    completionDetails.unavailableAttachments &&
    completionDetails.unavailableAttachments.length > 0
  ) {
 ...
 */
export function extractAllDeliveryPhotoUrls(
  taskDetails: WebhookTaskDetails
): string[] {
  const completionDetails = taskDetails.completionDetails || {};
  ...
  // Handle single photoUploadId
    ...
    photoUrls.push(
      `https://d15p8tr8p0vffz.cloudfront.net/${completionDetails.photoUploadId}/800x.png`
    );
  }
  // Fallback: unavailableAttachments (timing issue workaround)
  else if (
    ...
    for (const att of completionDetails.unavailableAttachments) {
      if (att.attachmentType === 'PHOTO' && att.attachmentId) {
        photoUrls.push(
          `https://d15p8tr8p0vffz.cloudfront.net/${att.attachmentId}/800x.png`
        );
      }
    }
  }
 ...
 */
export async function ensurePackingSupplyTrackingToken(
  orderId: number
): Promise<string> {
  ...
  const trackingToken = generateShortId();
 ...
 */
export function calculateIndividualTaskMetrics(
  taskDetails: WebhookTaskDetails
) {
  const completionDetails = taskDetails.completionDetails || {};
...
export function getWorkerName(
  worker?: string | WebhookWorkerData | null,
  assignedDriver?: any
): string {
 ...
 */
export async function updateTaskCompletionPhoto(
  taskDetails: WebhookTaskDetails
): Promise<void> {
...
export function buildTrackingUrl(
  ...
  type: 'appointment' | 'packing-supply' = 'appointment'
): string {
