...
export async function validateOnfleetSignature(
  ...
  if (!signature) {
    console.warn(
      '[Webhook] Missing X-Onfleet-Signature header — allowing request (Onfleet may omit on retries)'
    );
    return true;
  }
  ...
  if (!secret) {
    console.warn(
      '[Webhook] Missing ONFLEET_WEBHOOK_SECRET — skipping validation'
    );
    return true;
  }
  ...
  try {
    ...
    if (!isValid) {
      ...
      console.warn(
        `[Webhook] Signature mismatch (received: ${signature.substring(0, 16)}…, computed: ${hmac.substring(0, 16)}…, secret length: ${secret.length})`
      );
    }
    return true;
  } catch {
    console.warn(
      `[Webhook] Signature length mismatch (received: ${signature.length}, computed: ${hmac.length})`
    );
    return true;
  }
}
