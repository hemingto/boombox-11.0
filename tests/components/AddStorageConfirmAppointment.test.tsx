...
import {
  ...
  waitFor,
  fireEvent,
} from '../utils/AddStorageTestWrapper';
...
describe.skip('AddStorageConfirmAppointment', () => {
  ...
  describe('Component Rendering', () => {
    ...
    it('renders with proper semantic HTML structure', () => {
      ...
      expect(
        screen.getByRole('region', { name: 'Payment Information' })
      ).toBeInTheDocument();
    });
    ...
    it('renders back navigation button with proper accessibility', () => {
      ...
      expect(screen.getByTestId('chevron-left-icon')).toHaveAttribute(
        'aria-hidden',
        'true'
      );
    });
    ...
    it('renders description textarea with proper attributes', () => {
      ...
      expect(textarea).toHaveAttribute(
        'placeholder',
        'Let us know if there are stairs, narrow hallways, or tough parking conditions...'
      );
      expect(textarea).toHaveAttribute(
        'aria-label',
        'Location details and special instructions'
      );
  ...
  describe('Description Input', () => {
    ...
    it('calls onDescriptionChange when description is updated', async () => {
      ...
      fireEvent.change(textarea, {
        target: { value: 'New description with stairs and narrow hallway' },
      });

      expect(mockProps.onDescriptionChange).toHaveBeenCalledWith(
        'New description with stairs and narrow hallway'
      );
    });
    ...
    it('has proper fieldset structure for description', () => {
      ...
      const legend = screen.getByText(
        'Provide relevant information about your location'
      );
  ...
  describe('Payment Information Modal', () => {
    ...
    it('displays modal with proper content and accessibility', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('What if I need to reschedule?')
        ).toBeInTheDocument();
        expect(
          screen.getByText(/Please reschedule or cancel at least 48 hours/)
        ).toBeInTheDocument();
      });
    });
    ...
    it('modal content has proper heading structure', async () => {
      ...
      await waitFor(() => {
        const rescheduleHeading = screen.getByText(
          'What if I need to reschedule?'
        );
        expect(rescheduleHeading).toHaveAttribute('id', 'reschedule-heading');

        const rescheduleContent = screen.getByText(
          /Please reschedule or cancel at least 48 hours/
        );
        expect(rescheduleContent).toHaveAttribute(
          'aria-labelledby',
          'reschedule-heading'
        );
      });
    ...
    it('has proper modal description for screen readers', () => {
      ...
      expect(modalDescription).toHaveAttribute(
        'id',
      ...
      expect(paymentButton).toHaveAttribute(
        'aria-describedby',
        'payment-modal-description'
      );
    });
  ...
  describe('Payment Information Section', () => {
    it('has proper section structure with accessibility', () => {
      ...
      expect(paymentSection).toHaveAttribute(
        'aria-labelledby',
  ...
  describe('Accessibility', () => {
    it('meets WCAG 2.1 AA accessibility standards', async () => {
      const { container } = render(
        <AddStorageConfirmAppointment {...mockProps} />
      );
  ...
  describe('Edge Cases', () => {
    it('handles undefined description gracefully', () => {
      ...
      render(
        <AddStorageConfirmAppointment {...propsWithUndefinedDescription} />
      );
