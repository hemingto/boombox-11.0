...
jest.mock('@/components/forms', () => ({
  ...
  StorageUnitCounter: ({ onCountChange, initialCount }: any) => (
    <div data-testid="storage-unit-counter">
      ...
      <button
        onClick={() =>
          onCountChange(initialCount + 1, `${initialCount + 1} storage units`)
        }
      >
        Increase
      </button>
...
const mockFormState: AddStorageFormState = {
  ...
  pricing: {
    ...DEFAULT_ADD_STORAGE_FORM_STATE.pricing,
    loadingHelpPrice: '0',
...
const mockErrors: AddStorageFormErrors = { ...DEFAULT_ADD_STORAGE_FORM_ERRORS };
...
describe('AddStorageStep1', () => {
  ...
  describe('Component Rendering', () => {
    ...
    it('renders storage calculator link with proper accessibility', () => {
      ...
      const calculatorLink = screen.getByRole('link', {
        name: 'Open storage calculator in new tab',
      });
    ...
    it('renders plan details toggle button with proper accessibility', () => {
      ...
      const toggleButton = screen.getByRole('button', {
        name: 'Toggle plan details visibility',
      });
  ...
  describe('Address Input', () => {
    ...
    it('calls onAddressChange when address is updated', async () => {
      ...
      const lastCall =
        mockProps.onAddressChange.mock.calls[
          mockProps.onAddressChange.mock.calls.length - 1
        ][0];
      ...
      expect(lastCall.coordinates).toEqual({ lat: 40.7128, lng: -74.006 });
    });
  ...
  describe('Storage Unit Counter', () => {
    ...
    it('calls onStorageUnitChange when count is updated', async () => {
      ...
      expect(mockProps.onStorageUnitChange).toHaveBeenCalledWith(
        3,
        '3 storage units'
      );
    });
  ...
  describe('Plan Selection', () => {
    it('displays both plan options with icons', () => {
      ...
      expect(
        screen.getByText("No, I'll load my storage unit myself")
      ).toBeInTheDocument();
      expect(
        screen.getByText('Yes, I would love some help loading')
      ).toBeInTheDocument();
    ...
    it('shows DIY plan as selected by default', () => {
      ...
      const diyRadio = screen
        .getByTestId('radio-card-option1')
        .querySelector('input');
      const fullServiceRadio = screen
        .getByTestId('radio-card-option2')
        .querySelector('input');
    ...
    it('shows error state on icons when plan has error', () => {
      ...
      expect(screen.getByTestId('furniture-icon')).toHaveAttribute(
        'aria-invalid',
        'true'
      );
      expect(screen.getByTestId('moving-help-icon')).toHaveAttribute(
        'aria-invalid',
        'true'
      );
    });
  ...
  describe('Insurance Selection', () => {
    it('displays insurance options', () => {
      ...
      const insuranceSelect = screen
        .getByTestId('insurance-input')
    ...
    it('displays insurance error with proper accessibility', () => {
      const propsWithError = {
        ...
        errors: {
          ...DEFAULT_ADD_STORAGE_FORM_ERRORS,
          insuranceError: 'Please select insurance coverage',
        },
      };
  ...
  describe('Form Structure and Accessibility', () => {
    it('uses proper fieldset and legend structure', () => {
      ...
      // Address fieldset
      expect(
        screen.getByText('Where are we delivering your Boombox?')
      ).toBeInTheDocument();

      // Plan selection fieldset
      expect(
        screen.getByText('Do you need help loading your Boombox?')
      ).toBeInTheDocument();

      // Insurance fieldset
      expect(
        screen.getByText('Do you need additional insurance coverage?')
      ).toBeInTheDocument();
    });
  ...
  describe('Error Handling', () => {
    ...
    it('shows error states on form controls', () => {
      ...
      const addressInput = screen
        .getByTestId('address-input')
        .querySelector('input');
      const planRadio1 = screen
        .getByTestId('radio-card-option1')
        .querySelector('input');
      const planRadio2 = screen
        .getByTestId('radio-card-option2')
        .querySelector('input');
      const insuranceSelect = screen
        .getByTestId('insurance-input')
